...
To let a consumer (e.g. `web-app`, `jove`, `job-designer`) install a not-yet-mergeable WIP
commit without `github:mat3ra/jode#branch` (which has no way to build `dist/` on install),
CI publishes a pre-release tarball asset instead.
...
calls the reusable workflow at [`mat3ra/actions`](https://github.com/mat3ra/actions)'
`.github/workflows/release-wip.yml` on that push, which builds, packs, and publishes the
pre-release tarball asset for that commit. Commits without the marker don't trigger it,
so routine pushes stay quiet.
...
```text
https://github.com/mat3ra/jode/releases/download/wip-<short-sha>/jode.tgz
```
...
```bash
npm run mat3ra:install -- jode wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/jode@<release-url>` with
...
reusable workflow at `mat3ra/actions` and deletes any `wip-*` release whose commit is no
